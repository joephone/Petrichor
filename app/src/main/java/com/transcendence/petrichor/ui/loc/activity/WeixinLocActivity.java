package com.transcendence.petrichor.ui.loc.activity;
...
import com.transcendence.petrichor.ui.loc.adapter.AddressAdapter;
import com.transcendence.petrichor.ui.loc.utils.DataConversionUtils;
...
public class WeixinLocActivity extends PetrichorBaseActivity {
...
//    private ImageView      mIvSearch;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (null != data && SEARCHREQUESTCODE == requestCode) {
            try {
                userSelectPoiItem = (PoiItem) data.getParcelableExtra(Global.SP_KEY.SEARCH_INFO);
...
//                    refleshMark(userSelectPoiItem.getLatLonPoint().getLatitude(), userSelectPoiItem.getLatLonPoint().getLongitude());
    ...
    protected int getLayoutId() {
        return R.layout.activity_weixin_loc;
    }


    protected void initView() {
        setTitle(getString("微信发送定位"));
        setBackVisibility();
...
//        mIvSearch =  findViewById(R.id.iv_search);
        ...
        mBtSend = findViewById(R.id.tv_right);
        ...
        mAMap.setOnCameraChangeListener(new AMap.OnCameraChangeListener() {
            ...
            public void onCameraChangeFinish(CameraPosition cameraPosition) {
                if (null != location && null != cameraPosition && isSearchData) {
                    mIvLocation.setImageResource(R.mipmap.ic_loc_gps_black);
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtils.getInstance().put(Global.SP_KEY.LOCATION_INFO, gson.toJson(location));
                            doWhenLocationSucess();
                        } else {
                            //定位失败时，可通过ErrCode（错误码）信息来确定失败的原因，errInfo是错误信息，详见错误码表。
                            showToastWithErrorInfo(loc.getErrorCode());
                            L.d("location Error, ErrCode:"
...
//                        Toast.makeText(MainActivity.this, "定位", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.tv_right:
                        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
                            ...
                            Toast.makeText(WeixinLocActivity.this, "发送：" + poiItem.getTitle() + "  " + poiItem.getSnippet() + "  " + "纬度：" + poiItem.getLatLonPoint().getLatitude() + "  " + "经度：" + poiItem.getLatLonPoint().getLongitude(), Toast.LENGTH_SHORT).show();
                        }
                        startActivity(new Intent(WeixinLocActivity.this, MainActivity.class));
                        break;
                }
            }
        };
    ...
    private void refleshLocationMark(double latitude, double longitude) {
        if (mLocationGpsMarker == null) {
            mLocationGpsMarker = mAMap.addMarker(new MarkerOptions()
                    .position(new LatLng(latitude, longitude))
                    .icon(BitmapDescriptorFactory.fromBitmap(BitmapFactory
                            .decodeResource(getResources(), R.mipmap.ic_marker_init)))
                    .draggable(true));
        }
...
//        mAMap.invalidate();
    ...
    private void showToastWithErrorInfo(int error) {
        ...
        Toast.makeText(WeixinLocActivity.this.getApplicationContext(), tips, Toast.LENGTH_LONG).show();
    }
